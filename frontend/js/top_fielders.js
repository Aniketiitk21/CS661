...
export function drawTopFieldersChart(containerSelector, apiEndpoint = "/api/top_fielders") {
  ...
  if (select.empty()) {
    console.warn("⚠️  No global #global-season-select found for Top Fielders chart; defaulting to all seasons");
  } else {
    ...
    svg.selectAll(".bar")
      ...
      .on("mouseover",(e,d)=>{
        tooltip
          ...
          .html(`<strong>${d.fielder}</strong><br/>Attempts: ${d.fielding_attempts}`);
      })
